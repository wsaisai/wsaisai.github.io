	...
	$(function() { //加载完后的状态
		...
		$('#File').on('mousedown', function(ev) {
			...
			if($(target).hasClass('file_item') || $(target).parents().hasClass('file_item')) {
				//添加状态如果file_item有checked状态是选中的那么这个状态就可以拿来做判断
...
//在文件区域move的时候
			$('#File').on('mousemove', function(ev) {
					if(isChecked) {
						...
						//存储下谁是选中的,然后下面在拖动缩影的时候,以存的数组的长度生成缩影下的几个文件名
						var selectArr = who();
...
//			$('#File').html(createFileHtml(fileid));
//			$('#File').find('input').eq(0).parent().css('display', 'block');
//			$('#File').find('input').eq(0).focus()
//			$('#File').find('input').eq(0).on('mousedown', function(ev) {
//				return false;
//			})
//			$(document).on('mousedown', function() {
//				if($('#File').find('input').eq(0).show()) {
//					//title的内容为数据添加的标签
//					var Value = handle.getChildsById(datas, fileid).some(function(item) {
//						return item.title = $('#File').find('input').eq(0).val();
//					})
//					var isVal = $('#File').find('input').eq(0).val().trim();
//
//				}
//			})
//		})
